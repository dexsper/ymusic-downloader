...
- **Сплэш** — показывает логотип ~2 с, ожидает проверки токена, затем переходит дальше
- **Авторизация** — если аккаунт не привязан, предлагает войти через браузер; код для подтверждения можно скопировать кликом
- **Главный экран** — боковая колонка с логотипом и островком профиля, по центру — очередь загрузок
- **Настройки** — открываются модальным окном через меню профиля
